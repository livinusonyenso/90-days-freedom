...
function MobileCategoryDrawer({ activeCategoryId }: { activeCategoryId: string }) {
    ...
    <div className="mobile-category-drawer">
      <button
        ...
        style={{
          ...
          color: "#374151", cursor: "pointer", marginBottom: "0.75rem",
        }}
      >
      ...
      {open && (
        <div style={{ border: "1px solid #e5e7eb", borderRadius: "10px", padding: "0.5rem", marginBottom: "1rem", background: "white" }}>
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "2px" }}>
            {categories.filter(c => c.id !== "all").map((cat) => {
              const isActive = cat.id === activeCategoryId;
              return (
    ...
    <div style={{ border: "1px solid #e5e7eb", borderRadius: "8px", overflow: "hidden", marginBottom: "8px" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "0.65rem", minWidth: 0 }}>
          <GreenCheck size={18} />
          <span style={{ fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "0.88rem", color: "#111827", textAlign: "left" }}>
            {mod.title}
          </span>
          ...
          {mod.lessons.map((lesson) => (
            <div
              ...
              style={{
                ...
                borderTop: "1px solid #f3f4f6", gap: "0.5rem",
              }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: "0.65rem", minWidth: 0 }}>
                <GreenCheck size={16} />
                <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.82rem", color: "#374151", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {lesson.title}
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
                ...
                <span style={{ fontFamily: "'Inter',sans-serif", fontSize: "0.75rem", color: "#9ca3af" }}>{lesson.duration}</span>
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .right-column { display: block; width: 240px; flex-shrink: 0; position: sticky; top: 80px; }
        .mobile-enroll-card { display: none; }

        .learn-grid { display: grid; grid-template-columns: 1fr auto; gap: 2rem; align-items: start; }
        .reviews-grid { display: grid; grid-template-columns: 1fr auto; gap: 2rem; align-items: start; }
        .skills-box { min-width: 200px; border: 1px solid #e5e7eb; border-radius: 10px; padding: 1.25rem; }
        .included-box { min-width: 210px; }

        .filter-bar { display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.1rem; flex-wrap: wrap; }
        .filter-search { flex: 1; min-width: 180px; max-width: 380px; display: flex; align-items: center; gap: 0.5rem; border: 1px solid #e5e7eb; border-radius: 8px; padding: 0.4rem 0.85rem; }

        .detail-layout { display: flex; gap: 1.75rem; padding-top: 1.75rem; padding-bottom: 4rem; align-items: flex-start; }

        .action-buttons { display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem; margin-bottom: 1.5rem; }

        /* Tablet: hide sidebar + right column */
        @media (max-width: 1024px) {
          .desktop-sidebar { display: none; }
          .mobile-category-drawer { display: block; }
          .right-column { display: none; }
          .mobile-enroll-card { display: block; }
          .detail-layout { padding-bottom: 5rem; }
        }

        /* Mobile: stack overview grids */
        @media (max-width: 768px) {
          .learn-grid { grid-template-columns: 1fr; }
          .reviews-grid { grid-template-columns: 1fr; }
          .skills-box { min-width: unset; width: 100%; }
          .included-box { min-width: unset; width: 100%; }
          .filter-bar { gap: 0.5rem; }
          .filter-search { min-width: 100%; max-width: 100%; order: -1; }
          .detail-layout { padding-top: 1rem; }
        }

        /* Small phones: single-col action buttons */
      ...
      <div style={{ background: "white", minHeight: "100vh", fontFamily: "'Inter',sans-serif" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" style={{ paddingTop: "1.25rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontSize: "0.8rem", marginBottom: "0.6rem", flexWrap: "wrap" }}>
          ...
          <h1 style={{ fontWeight: 800, fontSize: "clamp(1.4rem,3vw,2.2rem)", color: "#111827", lineHeight: 1.2, marginBottom: "0.5rem" }}>
            {course.title}
          </h1>
        ...
        {/* ── 3-Column Layout ── */}
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 detail-layout">
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {/* Mobile category drawer */}
            <MobileCategoryDrawer activeCategoryId={course.category} />
            ...
            <div className="filter-bar">
              ...
              {["All Levels", "Sort by: Most Popular"].map((label, i) => (
                <button key={i} style={{ display: "flex", alignItems: "center", gap: "5px", border: "1px solid #e5e7eb", borderRadius: "8px", padding: "0.4rem 0.85rem", fontFamily: "'Inter',sans-serif", fontSize: "0.8rem", color: "#374151", background: "white", cursor: "pointer", whiteSpace: "nowrap" }}>
                  {label}
            ...
            <div className="action-buttons">
              <a href="https://codewalker.gumroad.com/" target="_blank" rel="noopener noreferrer" style={{ background: "#14532d", color: "white", border: "none", borderRadius: "8px", padding: "0.75rem 0", fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "0.9rem", cursor: "pointer", textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center" }}>
                Enroll Now
              </a>
            ...
            {/* Mobile instructor + enroll card */}
            <div className="mobile-enroll-card" style={{ marginBottom: "1.5rem" }}>
              <div style={{ border: "1px solid #e5e7eb", borderRadius: "12px", overflow: "hidden" }}>
                <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <p style={{ margin: 0, fontWeight: 700, fontSize: "0.85rem", color: "#111827" }}>{detail.instructor.name}</p>
                    <p style={{ margin: "2px 0 0", fontSize: "0.72rem", color: "#6b7280" }}>{detail.instructor.role}</p>
                  </div>
                ...
                <div style={{ padding: "1rem 1.25rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                  <a href="https://codewalker.gumroad.com/" target="_blank" rel="noopener noreferrer" style={{ flex: 1, background: "#14532d", color: "white", border: "none", borderRadius: "8px", padding: "0.7rem 0", fontFamily: "'Inter',sans-serif", fontWeight: 700, fontSize: "0.9rem", cursor: "pointer", textDecoration: "none", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    Enroll Now
                  </a>
                  <span style={{ fontSize: "0.72rem", color: "#9ca3af", whiteSpace: "nowrap" }}>30-Day Guarantee</span>
                </div>
              </div>
            </div>

            {/* Tabs */}
            <div style={{ display: "flex", gap: "1.75rem", borderBottom: "1px solid #e5e7eb", marginBottom: "2rem", overflowX: "auto" }}>
            ...
            {activeTab === "overview" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "2.5rem" }}>
                ...
                <div style={{ border: "1px solid #e5e7eb", borderRadius: "10px", overflow: "hidden" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.25rem", borderBottom: "1px solid #e5e7eb", background: "white", gap: "0.75rem", flexWrap: "wrap" }}>
